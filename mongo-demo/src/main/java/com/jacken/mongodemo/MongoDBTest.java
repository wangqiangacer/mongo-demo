...
public class MongoDBTest {
    ...
    public static void main(String args[] ){
        try{
            ...
            ServerAddress serverAddress = new ServerAddress("49.232.173.109",27017);
            //MongoCredential.createScramSha1Credential()三个参数分别为 用户名 数据库名称 密码
            MongoCredential credential = MongoCredential.createScramSha1Credential("admin", "config", "admin".toCharArray());
            //通过连接认证获取MongoDB连接
    ...
    public static void find(MongoDatabase a){
        try {
            ...
            while (mongoCursor.hasNext()) {
                System.err.println("查询的数据:" + mongoCursor.next());
            }
        }catch(Exception e){
    ...
    public static void delete(MongoDatabase a){
        try {
            ...
            MongoCursor<Document> mongoCursor = findIterable.iterator();
            while (mongoCursor.hasNext()) {
                System.err.println("删除后的数据：" + mongoCursor.next());
            }
        }catch(Exception e){
    ...
    public  static  void getCount(MongoDatabase a){
        MongoCollection<Document> mongoCollection = a.getCollection("user");
